        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                if (disposing)
                {
                    ...
                    if (Value != null)
                    {
                        // Defensive sanitization: avoid forcing CLR to marshal non-COM-typelib value types
                        // into a VARIANT (which throws NotSupportedException for types such as System.Drawing.Point).
                        try
                        {
                            var valueType = Value.GetType();

                            // If it's a non-primitive value type (struct) that the marshaller can't handle,
                            // convert it to a managed-safe representation instead of calling VariantClear.
                            // Special-case System.Drawing.Point which is observed to cause the Variant marshalling failure.
                            if (valueType == typeof(System.Drawing.Point) ||
                                (valueType.IsValueType && !valueType.IsPrimitive && valueType != typeof(decimal)))
                            {
                                try
                                {
                                    // Prefer a stable textual representation; if that fails, drop the value.
                            ...
                            {
                                // For types where VariantClear is appropriate, call it and null out the field.
                                ...
                                {
                                    // If VariantClear reports failure, fall back to null to avoid leaks.
                                    Value = null;
                                }
                            }
                        }
                        ...
                        {
                            // Marshal attempted and failed; ensure we don't propagate exception from disposal.
                            Value = null;
                        }
                        catch (Exception)
                        {
                            // Be conservative on any unexpected errors during dispose: drop the value.
                            Value = null;
                        }
                    }
                }
